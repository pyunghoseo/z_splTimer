...
INITIAL_TIMES = [118, 113, 113, 113, 113]
reps = 0
timer = None
...
def start_timer():
    ...
    if reps < len(INITIAL_TIMES) * 2:
        if reps % 2 == 0:
            ...
            # WAVE 사이의 10초 인터벌
            count_down(10, -1)  # 인터벌 카운트다운, wave 인덱스 -1로 설정
        reps += 1
    else:
        # 모든 WAVE 완료 후 종료 처리
        reset_timer()
...
def count_down(count, wave):
    global current_time, timer, reps
    current_time = count
    count_sec = f"{count}"
    canvas.itemconfig(timer_text, text=count_sec)
    ...
    if count > 0:
        ...
        # 현재 Wave 종료 시
        if reps < len(INITIAL_TIMES) * 2 - 1:  # 마지막 Wave가 아닌 경우
            reps += 1  # 다음 Wave 또는 인터벌로 넘어가기 위해 reps 증가
            if reps % 2 == 0:
                wave_label.config(text=f"WAVE {reps // 2 + 1}")  # WAVE 레이블 업데이트
            start_timer()  # 다음 Wave 또는 인터벌 시작
        else:
            # 모든 Wave가 완료된 경우
            reset_timer()  # 타이머 리셋 및 초기 상태로 복원
