...
def reset_timer():
    ...
    current_label.config(text="Timer")
    next_label.config(text="")
    global reps
    reps = 0
    canvas.itemconfig(timer_text, text="118")
...
current_label = Label(window, text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 35))
current_label.grid(column=1, row=0)

next_label = Label(window, text="", fg=PINK, bg=YELLOW, font=(FONT_NAME, 20))
next_label.grid(column=1, row=1)

# 캔버스 크기 조정 (기존 크기의 2배)
canvas = Canvas(window, width=400, height=448, bg=YELLOW, highlightthickness=0)
