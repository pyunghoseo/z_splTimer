...
def handle_danger_level_click(wave, level):
    # Reset the previously selected buttons to 'normal' state and default background color
    global selected_danger_level  # Ensure we're modifying the global variable
    ...
    for wave, buttons in buttons_info.items():
        ...
        for percent, column in buttons:
            ...
            if initial_selections.get(wave) == percent:
                handle_danger_level_click(wave, btn)
